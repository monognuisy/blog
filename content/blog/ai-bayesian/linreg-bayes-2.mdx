...
(<Link to="../linreg-freq">Linear Regression (1) - Frequentist</Link> 참고)

$$
{\bf w}^\top \left( {\bf x}{\bf x}^\top + XX^\top + \frac{\sigma^2}{\sigma^2_0}I \right){\bf w} - 2{\bf w}^\top \left(y{\bf x} + X{\bf y} \right) + y^2
$$

<Link to="../linreg-bayes-1">Linear Regression (2) - Bayesian의 Training</Link> 의 완전제곱식 공식을 참고하면, 다음과 같이 정리된다.
...
여기서 <Highlight color={textHighlightRed}>**빨간색 term**</Highlight>과 
<Highlight color={textHighlightBlue}>**파란색 term**</Highlight>의 합으로 나타낼 수 있다는 점에 주목하자.

<Highlight color={textHighlightRed}>**빨간색 term**</Highlight>을 
**Aleatoric uncertainty**라고 하며, 이는 underline density function의 uncertainty를 의미한다.

<Highlight color={textHighlightBlue}>**파란색 term**</Highlight>은 
...
즉, <Link to="../linreg-bayes-1">Linear Regression (2) - Bayesian의 Training</Link> 에서 관찰한 posterior 의 분산과 비슷하게 $N \to \infty$ 인 경우를 의미하는 것이라고 할 수 있다.
...
<Link to="../linreg-bayes-1">Linear Regression (2) - Bayesian의 Training</Link>
에서의 $\bm \mu_{\bf w}$ 를 이용하여 prediction 한 것으로도 생각할 수 있다.
